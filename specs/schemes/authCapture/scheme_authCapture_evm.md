...
| `captureAuthorizer`   | Yes      | `address`                | Address authorized to authorize/capture/void/refund/charge — must be `msg.sender` of the on-chain settle (the facilitator EOA, or any smart contract that ends up calling escrow). Committed on-chain as `PaymentInfo.operator`. |
