...
- **AuthCaptureEscrow**: Singleton — locks funds, enforces expiries, distributes on capture/refund. Audited canonical contract from `base/commerce-payments`, reused at the same address on every chain it's deployed to.
...
The escrow and token collectors are the audited, live `base/commerce-payments` deployment. Implementations MUST use these exact addresses; the same addresses are reused on every chain it's deployed to.
