...
The **captureAuthorizer** is the entity authorized to authorize, capture, void, refund, or charge a payment — i.e., the only address that can drive the on-chain payment lifecycle. In a facilitator-driven flow, that's either the facilitator itself or a smart contract the facilitator controls (per-network specifics in the per-network spec document). It is not the merchant address — the merchant only receives funds; lifecycle authority belongs to the captureAuthorizer.
