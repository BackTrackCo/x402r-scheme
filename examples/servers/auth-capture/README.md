...
Express resource server protected by the [auth-capture](../../../specs/schemes/auth-capture/scheme_auth_capture_evm.md) scheme. The server publishes payment requirements with all spec-mandated `extra` fields and delegates verify/settle to a configured facilitator.
...
This example demonstrates the authorize phase only. Capture, void, and refund are the captureAuthorizer's responsibility and are not handled by this server; refer to the [scheme spec](../../../specs/schemes/auth-capture/scheme_auth_capture_evm.md) for the protocol-level surface.
